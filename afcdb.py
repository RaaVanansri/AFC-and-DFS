...
incumbents = ["Fixed Earth Station","Common Carrier"]
    ...
    def uniband(band,frequency):
        ...
        for i,j in enumerate(band):
            ...
            for k in j:
                ...
                if i == 0:y.update({f'{k}':{"Band":"20Mhz","Frequency(Mhz)":f'{freq}-{freq+20}',"Service":incumbents[random.randint(0,1)],"Status":"Active" if avail == 1 else "Inactive"}});freq += 20
                elif i == 1:y.update({f'{k}':{f"Band":"40Mhz","Frequency(Mhz)":f'{freq}-{freq+40}',"Service":incumbents[random.randint(0,1)],"Status":"Active" if avail == 1 else "Inactive"}});freq += 40
                elif i == 2:y.update({f'{k}':{f"Band":"80Mhz","Frequency(Mhz)":f'{freq}-{freq+80}',"Service":incumbents[random.randint(0,1)],"Status":"Active" if avail == 1 else "Inactive"}});freq += 80
                elif i == 3:y.update({f'{k}':{f"Band":"160Mhz","Frequency(Mhz)":f'{freq}-{freq+160}',"Service":incumbents[random.randint(0,1)],"Status":"Active" if avail == 1 else "Inactive"}});freq += 160
